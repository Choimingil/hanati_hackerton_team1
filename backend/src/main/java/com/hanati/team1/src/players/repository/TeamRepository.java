...
public interface TeamRepository extends JpaRepository<Team, Long> {
	...
	@Query(
		"select new com.hanati.team1.src.players.models.GetPlayerListRes(p.playerId, tb.playerBacknum, p.playerName, (select pp.positionName from Position pp where pp.playerId = p.playerId and pp.positionLevel = 10), (select tt.tokenPrice from Trade tt where tt.playerId = p.playerId order by tt.tradeDate desc limit 1), p.playerProfile) "
			+ "from Player p "
